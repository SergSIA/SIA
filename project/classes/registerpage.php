...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		if( $this->cipherer->isFieldEncrypted("username") )
			$strUsername = $this->cipherer->MakeDBValue("username", $username, "", true);
		else
			$strUsername = make_db_value("username", $username);

		$sql = "select ".$this->getFieldSQLDecrypt("password")
			." from ". $this->connection->addTableWrappers( "public.user" ) 
			." where ".
		...
		$sql = "update ". $this->connection->addTableWrappers( "public.user" )
	...
	{			
		return projectURL() . GetTableLink( "register" )
			."?a=activate&u=".rawurlencode( base64_encode($this->regValues["username"]) )
	...
	protected function checkIfUsernameUnique( $strUsername )
	{
		...
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "public.user" ) 
			. " where " . 
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("username"), 
				$sUsername,
