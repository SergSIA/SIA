...
class RemindPasswordPage extends RunnerPage
{
 ...
 //RPM_RESET
	...
	protected function getUserSearchingSQLData()
	{
		...
		{
			$fullcUserNameField = $this->connection->addTableWrappers( "public.user" ).".".$this->connection->addFieldWrappers( $this->usernameFiled );
			$fullcEmailField = $this->connection->addTableWrappers( "public.user" ).".".$this->connection->addFieldWrappers( $this->emailField );
			$fullcPasswordField = $this->connection->addTableWrappers( "public.user" ).".".$this->connection->addFieldWrappers( $this->passwordFiled );
		}
		...
		$strSQLData["sql"] = $selectClause." from ".$this->connection->addTableWrappers("public.user")." where ".$sWhere;
